...
2. **Zero-knowledge at rest.** The DB stores only ciphertext. The admin/host
   cannot read stored history. All LLM traffic is **browser-direct**: the
   browser calls Anthropic with the user's own key (stored DEK-encrypted in
   the vault). **Plaintext never touches the app server at all** — it exists
   only in the browser and at Anthropic during inference. (The legacy proxy
   mode that relayed plaintext through server RAM was removed.)
3. **LLM backend = Anthropic API**, called from the browser. There is no
...
| LLM | browser → Anthropic with each user's own key (always) | same |
...
- No real LLM call has been exercised by the handoff author (needs a user's
  Anthropic key in the browser).
...
chat + compaction + memory: decrypt ─plaintext(TLS)───────────────▶ Anthropic
...
server/
  ...
  prompts.js           DEFAULT/SUMMARIZE/MEMORIZE prompts + THERAPY_STYLES catalog
  routes/
    ...
    config.js          /config — models, compaction tuning, prompt/style catalog
...
public/
  ...
  styles.css
test/crypto.roundtrip.test.js  test/prompts.test.js
...
# No server API key — each user pastes their own Anthropic key in the app.
...
`npm install`, `npm run check`, `npm run test:crypto`, `npm run test:prompts`
(tests need no DB).
...
- **Proxy mode removed** — the server-side LLM transport (`/api/chat`,
  ...
  `server/llm.js` became `server/prompts.js`, `routes/chat.js` became
  `routes/config.js` (/config only).
...
3. **Crisis handling.** A therapy tool will receive self-harm language. Zero-
   ...
   (`server/prompts.js`) and document that no human escalation exists.
...
7. **LLM cost controls** — none in-app; each user's own Anthropic key bears
   the spend, so caps live in the Anthropic console.
...
- **Server never stores, logs, or even receives plaintext.** Only ciphertext +
  non-sensitive metadata at rest; no route handles message plaintext (there is
  no LLM proxy). Keep the generic error handler.
